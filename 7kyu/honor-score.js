...
//
//     '2kyu' worth of 1 point, '1kyu' worth of 2 points
// So if for example:
//
// M has honor 11
// D has honor 2
...
function getHonorPath(honorScore, targetHonorScore) {
    const difference = targetHonorScore - honorScore;
    if(difference <= 0) {
        return 'Let warrior go for drinks!';
    } else if (difference === 1) {
        return `Needs to solve ${difference} 2kyus`;
    } else if (difference % 2 === 0) {
        return `Needs to solve ${difference / 2} 1kyus`;
    } else {
        return `Needs to solve ${(difference - 1) / 2} 1kyus and 1 2kyus`;
    }
}
